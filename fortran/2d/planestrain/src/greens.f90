...
CONTAINS
  ...
  CONTAINS
    ...
    SUBROUTINE tractionRows(xc,sv,dv,nv,m,n,rows)
      ...
      ! patches
      DO j=1,DGF_PATCH
         CALL computeTractionKernelsPlaneStrain(xc,sv,dv,nv, &
                 in%patch%ns,in%patch%x, &
                 in%patch%width,in%patch%dip, &
                 in%mu,in%lambda,KK(j,:,1),KK(j,:,2))
      END DO
  
      ! strain volumes
      DO j=1,DGF_VOLUME
         CALL computeTractionKernelsStrainVolumePlaneStrain(xc,sv,dv,nv, &
                 in%strainVolume%ns,in%strainVolume%x, &
                 in%strainVolume%thickness,in%strainVolume%width, &
                 in%strainVolume%dip, &
                 1d-3*eye(j,1),1d-3*eye(j,2),1d-3*eye(j,3),in%mu,in%lambda, &
                 KL(j,:,1),KL(j,:,2))
      END DO
    ...
    SUBROUTINE stressRows(xc,sv,dv,nv,m,n,rows)
      ...
      ! patches
      DO j=1,DGF_PATCH
         CALL computeStressKernelsPlaneStrain(xc,sv,dv,nv, &
                 in%patch%ns,in%patch%x, &
                 in%patch%width,in%patch%dip,in%mu,in%lambda, &
                 LK(j,:,1),LK(j,:,2),LK(j,:,3))
      END DO
  
      ! strain volumes
      DO j=1,DGF_VOLUME
         CALL computeStressKernelsStrainVolumePlaneStrain(xc,sv,dv,nv, &
                 in%strainVolume%ns,in%strainVolume%x, &
                 in%strainVolume%thickness,in%strainVolume%width, &
                 in%strainVolume%dip, &
                 1d-3*eye(j,1),1d-3*eye(j,2),1d-3*eye(j,3),in%mu,in%lambda, &
                 LL(j,:,1),LL(j,:,2),LL(j,:,3))
      END DO
          ...
          CASE (FLAG_PATCH)
             ...
             DO j=1,DGF_PATCH
                CALL computeDisplacementPlaneStrain( &
                        ...
                        1._8,in%mu,in%lambda, &
                        O(l+j-1,p),O(l+j-1,p+1))

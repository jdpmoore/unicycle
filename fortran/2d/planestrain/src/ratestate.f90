...
!! plane strain under the radiation damping approximation.
!!
!! \mainpage
!! 
...
!$  INTEGER :: omp_get_num_procs,omp_get_max_threads
    ...
    IF (0 .EQ. rank) THEN
       PRINT 2000
       PRINT '("# RATESTATE")'
       PRINT '("# quasi-dynamic earthquake simulation in plane strain")'
...
!$                omp_get_max_threads(),omp_get_num_procs()
       ...
       IF (in%patch%ns .GT. 0) THEN
          ...
          DO k=1,in%patch%ns
             ...
             PRINT '(I4,ES10.2E2,2ES9.2E1,2ES8.2E1)',i, &
                  in%patch%s(k)%Vpl, &
                  in%patch%x(2,k), &
                  in%patch%x(3,k), &
                  in%patch%width(k), &
...
2000 FORMAT ("# ----------------------------------------------------------------------------")
  ...
  SUBROUTINE printhelp()
    ...
    IF (0.EQ.rank) THEN
       PRINT '("usage:")'
       PRINT '("")'
       PRINT '("mpirun -n 2 unicycle-ps-ratestate [-h] [--dry-run] [--help] [--epsilon 1e-6] [--export-netcdf] [filename]")'
       PRINT '("")'
       PRINT '("options:")'
  ...
  SUBROUTINE printversion()
    ...
    IF (0.EQ.rank) THEN
       PRINT '("unicycle-ps-ratestate version 1.0.0, compiled on ",a)', __DATE__
       PRINT '("")'
       CALL FLUSH(6)
    END IF

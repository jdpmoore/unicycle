...
C                                                                       
      ...
      COMMON /C1/P,Q,S,T,XY,X2,Y2,D2,R,R2,R3,R5,QR,QRX,A3,A5,B3,C3,     
     *           UY,VY,WY,UZ,VZ,WZ                                      
c     COMMON /C1/P,Q,S,T,XY,X2,Y2,D2,R,R2,R3,R5,QR,QRX,A3,A5,B3,C3      
...
C-----                                                                  
      ...
      RETURN                                                            
      END                                                               
      SUBROUTINE  DCCON3(X,P,Q,AL1,AL2,AW1,AW2,KXI,KET)                 
      IMPLICIT REAL*8 (A-H,O-Z)                                         
      REAL*8  X,AL1,AL2,AW1,AW2                                         
      DIMENSION KXI(2),KET(2)                                           
C                                                                       
C************************************************************           
C*****   CHECK SINGULARITIES RELATED TO R+XI AND R+ET   *****           
C************************************************************           
C                                                                       
C***** INPUT                                                            
C*****   X,P,Q : STATION COORDINATE                                     
C*****   AL1,AL2,AW1,AW2 : FAULT DIMENSIONS                             
C***** OUTPUT                                                           
C*****   KXI(2) : =1 IF R+XI<EPS                                        
C*****   KET(2) : =1 IF R+ET<EPS                                        
C                                                                       
      DATA  EPS/1.D-6/                                                  
C-----                                                                  
      KXI(1)=0                                                          
      ...
      KET(1)=0                                                          
      KET(2)=0                                                          
      IF(X.GT.AL1 .AND. P.GT.AW2) RETURN                                
C-----                                                                  
      DO 222 K=1,2                                                      
      IF(K.EQ.1) ET=P-AW1                                               
      IF(K.EQ.2) ET=P-AW2                                               
      DO 111 J=1,2                                                      
        ...
        RXI=R+XI                                                        
        RET=R+ET                                                        
        IF(X.LT.AL1 .AND. RXI.LT.EPS) KXI(K)=1                          
        IF(P.LT.AW1 .AND. RET.LT.EPS) KET(J)=1                          
  111 CONTINUE                                                          
  222 CONTINUE                                                          
      RETURN                                                            
      END                                                               

function [Ks,Kd]=computeDisplacementKernelsOkada92(patch,mu,nu,x,vecsize)
...
for j=1:2
    ...
    for i=1:patch.N
        if 0==mod(i-1,2)
            textprogressbar(i/patch.N*50+(j-1)*50);
        end
        ...
        zd=x(:,3);
%         +zg;
        %[ux,uy,uz]=computeDisplacementOkada85(U,x,y,nu,delta,d,len,W,fault_type,strike)
        [ux,uy,uz]=computeDisplacementOkada92(1,xd,yd,zd, mu, nu,zg,dip,L,W,j,strike);
        %[ux,uy,uz]=computeDisplacementOkada92(U,X,Y,Z,G,nu,depth,angle,L,W,type,strike)
        switch(vecsize)
            case 2
                u=[ux,uy]';
            case 3

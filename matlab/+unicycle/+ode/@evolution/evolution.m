classdef evolution < handle
    ...
    methods
        ...
        function o=evolution(src,flt,shz,evts,varargin)
                    ...
                    if isobject(flt)
                        ...
                        if isempty(varargin)
                            ...
                            if ~exist(strcat(o.knlpath,'FK_ss.grd'),'file')
                                if ~exist(strcat(o.knlpath,'FK.mat'),'file')
                                    [o.FK{:}]=src.tractionKernels(flt);
                                    for i = 1:numel(o.FK)
                                        fname=strcat(o.knlpath,'FK_',o.knl.FK{i},'.grd');
                                        unicycle.export.grdwrite([0 1], [0 1],o.FK{i},fname)
                                    end
                                else
                                    load(strcat(o.knlpath,'FK.mat'));
                                    o.FK=sFK;
                                    clear sFK
                                    for i = 1:numel(o.FK)
                                        fname=strcat(o.knlpath,'FK_',o.knl.FK{i},'.grd');
                    ...
                    if isobject(shz)
                        ...
                        if isempty(varargin)
                            ...
                            if ~exist(strcat(o.knlpath,'FL_s11.grd','file'))
                                if ~exist(strcat(o.knlpath,'FL.mat','file'))
                                    [o.FL{:}]=src.stressKernels(shz);
                                    for i = 1:numel(o.FL)
                                        fname=strcat(o.knlpath,'FL_',o.knl.FL{i},'.grd');
                                        unicycle.export.grdwrite([0 1], [0 1],o.FL{i},fname)
                                    end
                                else
                                    load(strcat(o.knlpath,'FL.mat'));
                                    o.FL=sFL;
                                    clear sFL
                                    for i = 1:numel(o.FL)
                                        fname=strcat(o.knlpath,'FL_',o.knl.FL{i},'.grd');
            ...
            o.evt=unicycle.ode.evolution.eventKernels(evts,flt,shz,o.knlpath);
        end
        ...
        function [] = plotHorizontalProfiles(obj,depth,threshold,seismicPeriod,aseismicPeriod,component)
            ...
            offset=0;
            if isprop(obj,'flt')
                for k=1:length(obj.flt.segments)
                    % patch index
        ...
        function [] = plotIndexProfiles(obj,pos,threshold,seismicPeriod,aseismicPeriod,component,transpose)
            ...
            n=(obj.flt.x(pos(end),:)-obj.flt.x(pos(1),:))';
            n=n/sqrt(sum(n.^2));
            x=obj.flt.x(pos,:)*n;
        ...
        function [] = plotVerticalProfiles(obj,lo,threshold,seismicPeriod,aseismicPeriod,component)
            ...
            for k=1:length(obj.flt.segments)
                % along-strike coordinates
                x=(obj.flt.x-repmat(obj.flt.segments{k}.x,obj.flt.segments{k}.nPatch,1))*...
                    obj.flt.segments{k}.sv'+offset;
                
                % patch index
                pos=find(abs(x-lo)<=lMin/2);
                
                % down-dip coordinates
                y=(obj.flt.x(pos,:)-repmat(obj.flt.segments{k}.x,length(pos),1))*...
                    obj.flt.segments{k}.dv'+offset;
                ...
                if ~isempty(index2)
                    ...
                    pos2=getPeriodicIndex(obj.t(index2),aseismicPeriod);
                    if ~isempty(pos2)
                        plot(obj.y(index,index2(pos2)),y,'b--')
                    end
                else
                    fprintf('no velocity below %8.2e\n',threshold);
                end
    ...
    methods(Static)
        function evt=eventKernels(evts,flt,shz,knlpath)
                ...
                if isobject(flt)
                    ...
                    if exist(knlpath)      
                        fnameKK={[knlpath 'evt_' num2str(k) '_KK_ss.grd'],...
                            ...
                            [knlpath 'evt_' num2str(k) '_KK_ds.grd'],...
                ...
                if isobject(shz)
                    ...
                    if exist(knlpath)
                        fnameKL={[knlpath 'evt_' num2str(k) '_KL_s11.grd'],...
                            ...
                            [knlpath 'evt_' num2str(k) '_KL_s13.grd'],...
                            [knlpath 'evt_' num2str(k) '_KL_s22.grd'],...
                            [knlpath 'evt_' num2str(k) '_KL_s23.grd'],...
                            [knlpath 'evt_' num2str(k) '_KL_s33.grd'];...
                            [knlpath 'evt_' num2str(k) '_KL_d11.grd'],...
                            [knlpath 'evt_' num2str(k) '_KL_d12.grd'],...
                            [knlpath 'evt_' num2str(k) '_KL_d13.grd'],...
                            [knlpath 'evt_' num2str(k) '_KL_d22.grd'],...
                            [knlpath 'evt_' num2str(k) '_KL_d23.grd'],...
                            [knlpath 'evt_' num2str(k) '_KL_d33.grd']};
                        fnameKLmat=[knlpath 'evt_' num2str(k) '_KL.mat'];

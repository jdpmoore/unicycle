...
%   demo.m
%   $Id: demo.m 1079 2008-08-20 21:34:15Z ewout78 $
%
%   ----------------------------------------------------------------------
...
%   ----------------------------------------------------------------------
    ...
    opts = setParams('iterations',1000,'verbosity',0);
    for i=1:length(tau)
    ...
    opts = setParams('iterations',1000,'verbosity',1);
    AW   = A * spdiags(1./w,0,n,n);
    x = bp(AW, b, opts);
    x1 = x;                        % Reconstruct solution, no weighting
    
    opts = setParams('iterations',1000,'verbosity',1,'weights',w);
    x = bp(A, b, opts);
    ...
    opts = setParams('verbosity',1,'weights',weights);
    x    = mmv(A,B,0,opts);
    ...
    % Solve unweighted version
    opts = setParams('verbosity',1);
    x    = group(A*W,b,groups,0,opts);
    x1   = x;

    % Solve weighted version
    opts = setParams('verbosity',1,'weights',weights);
    x    = group(A,b,groups,0,opts);

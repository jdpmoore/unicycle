function [x,r,g,info] = mmv( A, B, sigma, options )
%MMV  Solve multi-measurement basis pursuit denoise (BPDN)
%
%   MMV is designed to solve the basis pursuit denoise problem
%
...
%                                  v = A'*w  if mode == 2. 
%   
%   X = MMV(A,B,SIGMA) solves the BPDN problem.  If SIGMA=0 or
%   SIGMA=[], then the basis pursuit (BP) problem is solved; i.e., the
%   constraints in the BPDN problem are taken as AX=B.
%
%   X = MMV(A,B,SIGMA,OPTIONS) specifies options that are set using
%   SETPARAMS.
%

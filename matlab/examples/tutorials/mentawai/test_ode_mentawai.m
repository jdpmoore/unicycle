...
rcv=geometry.receiver('./faults/sunda_30km_patch.flt',earthModel);
...
% Mentawai patch
index=find(1==omega((y-20e3)/300e3).*omega((rcv.xc(:,3)+35e3)/40e3));
...
src=geometry.source('./faults/slab_30km.flt',earthModel);
%src=geometry.source();
...
if ~exist('evl')
    ...
    evl=ode.rateandstate(src,rcv,evt,'./menta_30_kernels/');
else
...
if true
    ...
    hold on;box on
    evl.plotIndexProfiles((148:176),2e-8,1e-3,5,2,false)
...
if true
    ...
    hold on;box on
    evl.plotIndexProfiles([327 295 262 229 196 162 129 96 62 29 5],2e-8,1e-3,5,2,true)
...
% plot time series of fault slip and slip velocity
if true
    index={331,127};
    component=2;
    colors='rgbk';
...
if true
    ...
    for k=2:30:length(evl.t)
        ...
        box on, view([75 32]), xlabel('x (km)'), ylabel('y (km)')

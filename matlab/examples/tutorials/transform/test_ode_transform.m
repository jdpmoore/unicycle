...
%
%    src=geometry.source();
%    evt={geometry.coseismicpatch('./faults/coseismic.flt',0,earthModel)};
%
...
rcv.sigma=0*rcv.L+100;

fprintf('h* = %2.2e km\n',max(earthModel.G*rcv.l./(rcv.b-rcv.a)./rcv.sigma/1e3))
...
if ~exist('evl')
    ...
    evl=ode.rateandstate(src,rcv,evt,'./kernels_transform/');
else
...
if true
    ...
    hold on;box on
    depth=-mean(rcv.xc(:,3));
    evl.plotHorizontalProfiles(depth,5e-8,1e-4,1,1)
...
if true
    ...
    hold on;box on
    dist=mean(rcv.xc(:,2));
    evl.plotVerticalProfiles(dist,5e-8,1e-4,1,1)
...
% plot slip and velocity evolution
if false
    % bounds
    xlim=[-100 100]*1e3;
    ylim=[-50 250]*1e3;

...
yp=@(t,y) odeViscoelastic_single_fault(t,y,ss);
tic
% Solve the system
...
% Deviatoric stress in the mantle
Ta=sqrt(Y(:,ss.M*ss.dgfF+floor(ss.Nx/2)*ss.dgfS+1:ss.dgfS*ss.Nx:end)'.^2 +...
        Y(:,ss.M*ss.dgfF+floor(ss.Nx/2)*ss.dgfS+2:ss.dgfS*ss.Nx:end)'.^2);

% Effective viscosity in the mantle
...
h=colorbar('Location','NorthOutside','Position',[pos(1) pos(2)+pos(4)+0.03 pos(3) 0.015]);
caxis([min(min(log10(V))) max(max(log10(V)))]);
colormap(f1b,parula);
...
xTicks=(0:0.025:1)';
tTick=t(1+floor(xTicks*(length(t)-1)));
tSincePreviousTick=[0; tTick(2:end)-tTick(1:end-1)];
ax2 = axes('Position',ax1_pos,'XAxisLocation','top', ...
    'YAxisLocation','right','YTick',[],'Color','none');
set(ax2,'XTick',xTicks/xTicks(end),'XTickLabel', ...
    sprintf('%2.0e\n',tSincePreviousTick),'XTickLabelRotation',45);

f3b = subplot(4,1,2);cla;
pcolor(1:length(t)-1,ss.x3c/1e3,log10(Ep)), shading flat
%pcolor(1:length(t)-1,ss.x3c/1e3,log10(etaEffective*1e6)), shading flat
set(gca,'YDir','reverse');

caxis([log10(min(min(Ep))-realmin) log10(max(max(Ep))+realmin)]);
%caxis([log10(min(min(etaEffective*1e6))-realmin) 19.3])
h1=colorbar('Location','NorthOutside');
cmap=hot;
colormap(f3b,cmap);
title(h1,'Strain Rate (1/s)')
%title(h1,'Effective viscosity (Pa s)')
xlabel('Time Steps')
ylabel('Depth (km)');

f4b = subplot(4,1,3);cla;
plot(1:length(t)-1,log10(Vmax))
xlabel('Time Steps')
ylabel('Velocity (m/s) log10')
title('Maximum slip rates on fault')

f5b = subplot(4,1,4);cla;

...
if ~exist('evl','var')
    ...
    evl=ode.rateStrengtheningMaxwell(src,flt,shz,evt,'./triang_maxwell_kernels/');
    toc
else
...
if false
    ...
    box on;
    xlabel('East (m)');ylabel('North (m)'); zlabel('Up (m)');
    title('Shear stress change \sigma_{33}')
    set(gca,'view',[47 40]);
    axis equal
    fprintf('Sanity check: review geometry and properties before simulation\n');
    return
...
if true
    ...
    shz.plotShearZoneEdges();
    flt.plotPatch(sqrt(evl.y(1:evl.flt.dgf:(evl.flt.N*evl.flt.dgf),timeIndex).^2+evl.y(2:evl.flt.dgf:(evl.flt.N*evl.flt.dgf),timeIndex).^2));
    flt.plotSlipVectors(evl.y(1:evl.flt.dgf:(evl.flt.N*evl.flt.dgf),timeIndex),evl.y(2:evl.flt.dgf:(evl.flt.N*evl.flt.dgf),timeIndex),1e4)

...
% critical velocity Vdyn= 2 Cs a sigma / G
fprintf('# critical velocity %e m/s\n',2*sqrt(30e9/3e3)*1e-2*200e6/30e9);

% confining pressure (MPa)
flt.sigma=0*flt.l+300;
...
shz.x(:,3)=shz.x(:,3)-0.2*d;
shz.xc(:,3)=shz.xc(:,3)-0.2*d;
...
%            evl.LL{6,1}+evl.LL{6,4}+evl.LL{6,6};
    ...
    %flt.plotPatch();
    %flt.plotPatchIndex();
    %quiver3(flt.xc(:,1),flt.xc(:,2),flt.xc(:,3),...
    %    sc*(flt.sv(:,1).*cosd(flt.Vrake)+flt.dv(:,1).*sind(flt.Vrake)),...
    %    sc*(flt.sv(:,2).*cosd(flt.Vrake)+flt.dv(:,2).*sind(flt.Vrake)),...
    %    sc*(flt.sv(:,3).*cosd(flt.Vrake)+flt.dv(:,3).*sind(flt.Vrake)),0);
...
% integration options
options=ode.odeset('Refine',1,'RelTol',1e-13,'InitialStep',1e-5,'Stats','on','oDir','./postseismic_1');
tic
...
if true
    ...
    timeIndex=ceil(length(evl.t)/3);
    
    evt{1}.plotPatch();
    
    % strain
    toplot=evl.y(5:evl.shz.dgf:end,timeIndex)*1e6;

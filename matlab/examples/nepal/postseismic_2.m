...
flt.pinnedPosition=(evl.evt{1}.KK{1,2}*(evl.evt{1}.src.slip.*cosd(evt{1}.rake))+evl.evt{1}.KK{2,2}*(evl.evt{1}.src.slip.*sind(evt{1}.rake)))<0;

% initial conditions
y0=zeros(1,evl.flt.N*evl.flt.dgf+evl.shz.N*evl.shz.dgf);

% integration options
options=ode.odeset('Refine',1,'RelTol',1e-9,'InitialStep',1e-9,'Stats','on','oDir','./postseismic_2');
tic
...
if true
    ...
    for k=1:length(index)
        ...
        subplot(3,1,1);
        hold on, box on;
        toplot=gps.ua((stationId-1)*gps.vecsize+2,:)'*cosd(strike)+...
        ...
        subplot(3,1,2);
        hold on, box on;
        toplot=-(gps.ua((stationId-1)*gps.vecsize+2,:)'*cosd(azimuth)+...
        ...
        subplot(3,1,3);
        hold on, box on;
        toplot=gps.ua((stationId-1)*gps.vecsize+3,:)';
        plot(evl.t,toplot,'b-','lineWidth',1,'color',colors(stationId));
...
if true
    ...
    quiver(gps.x(:,1),gps.x(:,2), ...
           ...
           scale*gps.ua(2:gps.vecsize:end,timeIndex),0);
...
if true
    ...
    timeIndex=400;%ceil(length(evl.t)/1);
    ...
    % afterslip
    toplot=evl.y(2:evl.flt.dgf:(evl.flt.N*evl.flt.dgf),timeIndex);
    ...
    %set(gca,'xlim',xlim,'ylim',ylim,'zlim',[-145 0]);
    clim=get(gca,'clim');
    set(gca,'clim',[-1 1]*max(abs(clim(:)))/1);
    %set(gca,'clim',[-1 1]*5e-2);
    %box on, view([-227 42]), xlabel('x (km)'), ylabel('y (km)')
    grid on
end
